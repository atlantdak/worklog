...
**Goal:** Build a portable Claude Code plugin `worklog` that, on a user-supplied window, gathers GitHub facts, writes `.md` drafts for review, and — only after explicit approval — mirrors them into ClickUp via MCP and returns exact task links.
...
**Plugin root (and this plan/spec location, deliberately outside any shared repo):** `/path/to/worklog/` → becomes repository `atlantdak/worklog`.
...
**Files:**
- Create: `/path/to/worklog/.claude-plugin/plugin.json`
- Create: `/path/to/worklog/.claude-plugin/marketplace.json`
- Create: `/path/to/worklog/README.md`
- Create: `/path/to/worklog/.gitignore`
- Test: `/path/to/worklog/scripts/tests/test-manifests.sh`

- [ ] **Step 1: Write the failing test**

```sh
# /path/to/worklog/scripts/tests/test-manifests.sh
#!/usr/bin/env sh
set -eu
...
Run: `sh /path/to/worklog/scripts/tests/test-manifests.sh`
Expected: FAIL (files do not exist yet; jq errors).

- [ ] **Step 3: Create the manifests, README, .gitignore**

`/path/to/worklog/.claude-plugin/plugin.json`:
```json
{
  "name": "worklog",
...
`/path/to/worklog/.claude-plugin/marketplace.json`:
```json
{
  "name": "worklog",
  "description": "Local marketplace for the worklog end-of-day plugin.",
  "owner": {
    "name": "Dmytro Kishkin",
    "email": "atlantdak@gmail.com"
  },
...
```bash
cd /path/to/worklog && git init -q 2>/dev/null; git add .claude-plugin README.md .gitignore scripts/tests/test-manifests.sh
...
**Files:**
- Create: `/path/to/worklog/references/worklog.config.example.json`
- Create: `/path/to/worklog/references/format.md`
- Test: `/path/to/worklog/scripts/tests/test-references.sh`

- [ ] **Step 1: Write the failing test**

```sh
# /path/to/worklog/scripts/tests/test-references.sh
#!/usr/bin/env sh
set -eu
root="$(cd "$(dirname "$0")/../.." && pwd)"
fail=0
...
Run: `sh /path/to/worklog/scripts/tests/test-references.sh`
Expected: FAIL (files absent).

- [ ] **Step 3: Create the reference files**

`/path/to/worklog/references/worklog.config.example.json`:
```json
{
...
`/path/to/worklog/references/format.md`:
````markdown
...
**↳ Overview:** [★ Umbrella](UMBRELLA_URL)     ← umbrellas/standalone
**↳ Task:** [TASK-NN — title](PARENT_URL)      ← sub-entries
...
Terminology: never the word "epic"; use "task"/"subtask" (or the consuming project's
...
```bash
cd /path/to/worklog && git add references scripts/tests/test-references.sh
...
**Files:**
- Create: `/path/to/worklog/scripts/lib.sh`
- Test: `/path/to/worklog/scripts/tests/test-lib.sh`

- [ ] **Step 1: Write the failing test**

```sh
# /path/to/worklog/scripts/tests/test-lib.sh
#!/usr/bin/env sh
set -eu
root="$(cd "$(dirname "$0")/../.." && pwd)"
. "$root/scripts/lib.sh"
...
Run: `sh /path/to/worklog/scripts/tests/test-lib.sh`
...
```bash
cd /path/to/worklog && git add scripts/lib.sh scripts/tests/test-lib.sh
...
**Files:**
- Create: `/path/to/worklog/scripts/collect-window.sh`
- Create: `/path/to/worklog/scripts/tests/fixtures/gh-stub`
- Test: `/path/to/worklog/scripts/tests/test-collect-window.sh`

- [ ] **Step 1: Write the failing test + gh stub fixture**

`/path/to/worklog/scripts/tests/fixtures/gh-stub`:
```sh
#!/usr/bin/env sh
...
`/path/to/worklog/scripts/tests/test-collect-window.sh`:
```sh
#!/usr/bin/env sh
set -eu
root="$(cd "$(dirname "$0")/../.." && pwd)"
...
Run: `sh /path/to/worklog/scripts/tests/test-collect-window.sh`
...
```bash
cd /path/to/worklog && git add scripts/collect-window.sh scripts/tests/test-collect-window.sh scripts/tests/fixtures/gh-stub
...
**Files:**
- Create: `/path/to/worklog/scripts/validate-draft.sh`
- Create fixtures: `draft-ok.md`, `draft-baddate.md`, `draft-dupe.md`, `logged-prs.txt`
- Test: `/path/to/worklog/scripts/tests/test-validate-draft.sh`

- [ ] **Step 1: Write the failing test + fixtures**

`/path/to/worklog/scripts/tests/fixtures/logged-prs.txt`:
```
174
181
```

`/path/to/worklog/scripts/tests/fixtures/draft-ok.md`:
```
<!-- worklog:meta
{ "date":"2026-06-04",
  "entries":[
...
`/path/to/worklog/scripts/tests/fixtures/draft-baddate.md` (in-progress with a due date — must fail):
```
<!-- worklog:meta
{ "date":"2026-06-04",
  "entries":[
...
`/path/to/worklog/scripts/tests/fixtures/draft-dupe.md` (reuses already-logged PR 181 — must fail):
```
<!-- worklog:meta
{ "date":"2026-06-04",
  "entries":[
...
`/path/to/worklog/scripts/tests/test-validate-draft.sh`:
```sh
#!/usr/bin/env sh
set -eu
root="$(cd "$(dirname "$0")/../.." && pwd)"
...
Run: `sh /path/to/worklog/scripts/tests/test-validate-draft.sh`
...
```bash
cd /path/to/worklog && git add scripts/validate-draft.sh scripts/tests/test-validate-draft.sh scripts/tests/fixtures
...
**Files:**
- Create: `/path/to/worklog/commands/log-day.md`
- Test: `/path/to/worklog/scripts/tests/test-command.sh`

- [ ] **Step 1: Write the failing test**

```sh
# /path/to/worklog/scripts/tests/test-command.sh
#!/usr/bin/env sh
set -eu
...
Run: `sh /path/to/worklog/scripts/tests/test-command.sh`
Expected: FAIL (file absent).

- [ ] **Step 3: Write the command**

`/path/to/worklog/commands/log-day.md`:
```markdown
---
...
```bash
cd /path/to/worklog && git add commands/log-day.md scripts/tests/test-command.sh
...
**Files:**
- Create: `/path/to/worklog/skills/worklog-day/SKILL.md`
- Test: `/path/to/worklog/scripts/tests/test-skill.sh`

- [ ] **Step 1: Write the failing test**

```sh
# /path/to/worklog/scripts/tests/test-skill.sh
#!/usr/bin/env sh
set -eu
...
check "states no-write-before-approval" "grep -qi 'never write' '$f'"
exit $fail
```

- [ ] **Step 2: Run test to verify it fails**

Run: `sh /path/to/worklog/scripts/tests/test-skill.sh`
Expected: FAIL (file absent).

- [ ] **Step 3: Write the skill**

`/path/to/worklog/skills/worklog-day/SKILL.md`:
````markdown
---
name: worklog-day
...
- Never put the word "epic" (or the project's `terminology.avoid`) in any task.
...
```bash
cd /path/to/worklog && git add skills/worklog-day/SKILL.md scripts/tests/test-skill.sh
...
**Files:**
- Create: `/path/to/worklog/scripts/tests/run-all.sh`

- [ ] **Step 1: Write the aggregate runner**

```sh
# /path/to/worklog/scripts/tests/run-all.sh
#!/usr/bin/env sh
set -eu
d="$(cd "$(dirname "$0")" && pwd)"
fail=0
...
Run: `sh /path/to/worklog/scripts/tests/run-all.sh`
Expected: `ALL GREEN`.
...
In Claude Code:
```
/plugin marketplace add /path/to/worklog
/plugin install worklog@worklog
```
...
```bash
cd /path/to/worklog && git add scripts/tests/run-all.sh
git commit -m "test: aggregate test runner"
```

---

## Task 9 (user-gated, shared repo): Example Project draft-tracking cleanup

> Runs in the **shared** Example Project repo, not the plugin. Its own `chore/` branch + PR. The
...
**Files:**
- Modify: `<project-repo>/.gitignore`
- Untrack: `<project-repo>/ClickUp/_daily/`

- [ ] **Step 1: Confirm tasks-ua is tracked**

Run: `cd "/path/to/project" && git ls-files ClickUp/_daily | head`
...
```bash
git rm -r --cached ClickUp/_daily
...
Run: `ls ClickUp/_daily | head && git ls-files ClickUp/ | wc -l`
...
## Task 10 (user-gated): Publish the local plugin
...
gh repo create atlantdak/worklog --source /path/to/worklog --push
```
Expected: repo `atlantdak/worklog` (local) created; initial history pushed.
...
/plugin install worklog@worklog
```
Expected: installs from the local remote; `/log-day` available globally.

- [ ] **Step 3: First real end-to-end run (user-supervised)**

Run `/log-day yesterday` in Example Project, review the draft at the gate, approve, confirm the returned
...
- §2 form + privacy + neutral name + gitignore + Example Project cleanup → Tasks 1, 7 (onboarding gitignore), 9, 10. ✓
...
**Type/name consistency:** `wl_run_dir`/`wl_die`/`wl_need` defined in Task 3, used in 4/5. `worklog:meta` block + fields (`target/title/sp/status/start/due/parent/prs/links`) consistent across format.md (2), validator (5), skill (7). Scope kinds `date|pr-single|pr-range|since` consistent across command/skill/collect-window. Marketplace name `worklog` consistent across manifest (1) and install steps (8,10). ✓
